...
import { GroupError, transferGroupOwnership } from '@/lib/groups';
...
export async function POST(request: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    const group = await transferGroupOwnership(user, id, String(body.nextOwnerId || '').trim());
    ...
    console.error('GROUP OWNERSHIP POST ERROR:', e);
